	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.weixiudingdanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('weixiudingdan', this.weixiudingdanId);
                    this.detail = res.data
					this.detail.weixiudingdanContent = this.detail.weixiudingdanContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.weixiudingdanPhoto ? this.detail.weixiudingdanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('weixiudingdanYuyue', {
									yonghuId: _this.user.id,
									weixiudingdanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/weixiudingdanYuyue/list');
							}
						}
					});
				},
