	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0 10rpx 0px rgba(0,0,0,0)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"40rpx","borderWidth":"2rpx 2rpx 10rpx 2rpx","fontSize":"26rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.weixiuyuanName" type="text" placeholder="维修员姓名"></input>
		</view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.weixiuyuanName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('weixiuyuan','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <!-- 当前表 -->
                 <view v-show="item.weixiuyuanPhone" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">维修员手机号</text>
                     <text style="width: 70%">
                         <text v-if="item.weixiuyuanPhone != null">{{item.weixiuyuanPhone}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.weixiuyuanIdNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">维修员身份证号</text>
                     <text style="width: 70%">
                         <text v-if="item.weixiuyuanIdNumber != null">{{item.weixiuyuanIdNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 ...
                 <view v-show="item.weixiuyuanEmail" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">维修员邮箱</text>
                     <text style="width: 70%">
                         <text v-if="item.weixiuyuanEmail != null">{{item.weixiuyuanEmail}}</text>
                         <text v-else></text>
                     </text>
                 </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('weixiuyuan','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('weixiuyuan','审核') && item.weixiuyuanYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
