	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('weixiuyuyue', this.weixiuyuyueId);
                    this.detail = res.data
					this.detail.weixiuyuyueContent = this.detail.weixiuyuyueContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.weixiuyuyuePhoto ? this.detail.weixiuyuyuePhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('weixiuyuyueYuyue', {
									yonghuId: _this.user.id,
									weixiuyuyueId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/weixiuyuyueYuyue/list');
							}
						}
					});
				},
